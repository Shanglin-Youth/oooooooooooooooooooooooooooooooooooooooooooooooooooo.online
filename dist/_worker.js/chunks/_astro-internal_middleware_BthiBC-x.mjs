...
import { A as AstroError, R as ResponseSentError } from './astro/env-setup_zueCsFsd.mjs';
...
async function isSafeUrl(url, DoH = "https://family.cloudflare-dns.com/dns-query") {
  let safe = false;
  try {
...
async function onRequest$1(context, next) {
  ...
  if (matchedPath) {
    try {
      const DoH = context?.locals?.runtime?.env?.DOH || {"BASE_URL": "/", "MODE": "production", "DEV": false, "PROD": true, "SSR": true, "SITE": undefined, "ASSETS_PREFIX": undefined}.DOH;
